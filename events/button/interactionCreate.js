  ...
  try {
    ...
    if (interaction.customId === "premium") {
      ...
      await interaction.followUp({
        embeds: [new EmbedBuilder().setTitle(`${client.emotes.premium}| **Premium Info**`).setColor(client.colors.aqua).setDescription(`This bot is made by the owner of The doughy delight so ask him for more info.`)],
        components: [new ActionRowBuilder().addComponents([new ButtonBuilder().setStyle(ButtonStyle.Link).setEmoji(client.emotes.premium).setLabel("Buy Premium")])]
      })
    }
